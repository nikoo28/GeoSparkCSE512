...
public class JoinQuery implements Serializable {
  ...
  public static JavaPairRDD<Envelope, HashSet<Point>> SpatialJoinQueryUsingCartesianProduct(JavaSparkContext sc, PointRDD objectRDD, RectangleRDD queryWindowRDD) throws Exception {

    JavaRDD<Object> rawSpatialRDD = queryWindowRDD.getRawSpatialRDD();
    ...
    for (Object rectangle : rectangleList) {
      ...
      HashSet<Point> hashSet = new HashSet(pointList);
      collection2.add(new Tuple2<>((Envelope) rectangle, hashSet));
    }

    JavaPairRDD<Envelope, HashSet<Point>> envelopeHashSetJavaPairRDD = sc.parallelizePairs(collection2);
    return envelopeHashSetJavaPairRDD;
  }
